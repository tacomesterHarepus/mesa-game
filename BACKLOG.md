...
- **Resource allocation skip warning** — When humans click "Start Mission" without having allocated any CPU or RAM from the mission pool, show a confirmation dialog: "Continue without allocating resources? AIs will play with their base stats." Prevents accidentally skipping the allocation step, which happened during 2026-05-05 solo playtest. Priority: medium. Related to the "make pool distribution more obvious" entry above — the confirmation dialog is the minimum viable fix; the visual affordance improvements are the polish layer. *(Reported: 2026-05-05 solo playtest)*
